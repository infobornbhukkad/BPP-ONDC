...
@Service
public class UpdateServiceBuyer
{
    ...
    public ResponseEntity<String> update(final Schema request) throws JsonProcessingException {
        UpdateServiceBuyer.log.info("Going to validate json request before sending to seller...");
    ...
    private String sendRequestToSeller(final Schema request) {
        ...
        UpdateServiceBuyer.log.info("going to call seller id: {} with action: {}", (Object)context.getBppId(), (Object)action);
        final String url = context.getBppUri();
        final String json = this.jsonUtil.toJson((Object)request);
        UpdateServiceBuyer.log.info("final json to be send {}", (Object)json);
        final ConfigModel configModel = this.configService.loadApplicationConfiguration(context.getBapId(), "update");
    ...
    static {
        log = LoggerFactory.getLogger((Class)UpdateServiceBuyer.class);
    }
}
