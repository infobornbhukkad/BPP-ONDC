...
@RestController
public class OnUpdateControllerBuyer
{
    ...
    @PostMapping({ "/buyer/adaptor/on_update" })
    public ResponseEntity<String> onUpdate(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
        OnUpdateControllerBuyer.log.info("The body in {} adaptor is {}", (Object)"update", (Object)this.jsonUtil.unpretty(body));
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnUpdateControllerBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            final Context context = model.getContext();
            final String bppId = context.getBppId();
            final ConfigModel configuration = this.configService.loadApplicationConfiguration(context.getBapId(), "update");
            final boolean authenticate = configuration.getMatchedApi().isHeaderAuthentication();
            OnUpdateControllerBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnUpdateControllerBuyer.class);
    }
}
