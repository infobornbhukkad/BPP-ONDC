...
@RestController
public class OnConfirmControllerLogisticBuyer
{
    ...
    public ResponseEntity<String> onConfirm(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
    	OnConfirmControllerLogisticBuyer.log.info("The body in {} adaptor is {}", (Object)"confirm", body);
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnConfirmControllerLogisticBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            ...
            OnConfirmControllerLogisticBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
   ...
   public void getMySQLDataSource(final String body, final OnSchema request) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnConfirmControllerLogisticBuyer.class);
    }
}
