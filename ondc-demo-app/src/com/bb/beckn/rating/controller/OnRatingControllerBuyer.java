...
package com.bb.beckn.rating.controller;
...
import com.bb.beckn.rating.extension.OnSchema;
...
@RestController
public class OnRatingControllerBuyer
{
    ...
    @PostMapping({ "/buyer/adaptor/on_rating" })
    public ResponseEntity<String> onRating(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
        OnRatingControllerBuyer.log.info("The body in {} adaptor is {}", (Object)"rating", (Object)this.jsonUtil.unpretty(body));
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnRatingControllerBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            final Context context = model.getContext();
            final String bppId = context.getBppId();
            final ConfigModel configuration = this.configService.loadApplicationConfiguration(context.getBapId(), "rating");
            final boolean authenticate = configuration.getMatchedApi().isHeaderAuthentication();
            OnRatingControllerBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnRatingControllerBuyer.class);
    }
}
