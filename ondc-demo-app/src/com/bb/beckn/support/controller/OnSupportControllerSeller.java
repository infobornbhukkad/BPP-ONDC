...
@RestController
public class OnSupportControllerSeller
{
    ...
    @PostMapping({ "/seller/adaptor/on_support" })
    public ResponseEntity<String> onSupport(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
        OnSupportControllerSeller.log.info("The body in {} adaptor is {}", (Object)"support", (Object)this.jsonUtil.unpretty(body));
        try {
            ...
            final ConfigModel appConfigModel = this.configService.loadApplicationConfiguration(bppId, "support");
            final HttpHeaders headers = this.authHeaderBuilder.buildHeaders(body, appConfigModel);
            OnSupportControllerSeller.log.info("response in seller controller[which is will now send back to buyer adaptor] is {}", (Object)model.toString());
            this.sender.send(bapUrl, headers, body, (ApiParamModel)null);
        }
        catch (Exception e) {
            OnSupportControllerSeller.log.error("Error while sending request back to buyer adaptor:", (Throwable)e);
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnSupportControllerSeller.class);
    }
}
