...
@RestController
public class OnTrackControllerBuyer
{
    ...
    @PostMapping({ "/buyer/adaptor/on_track" })
    public ResponseEntity<String> onTrack(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
        OnTrackControllerBuyer.log.info("The body in {} adaptor is {}", (Object)"track", (Object)this.jsonUtil.unpretty(body));
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnTrackControllerBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            final Context context = model.getContext();
            final String bppId = context.getBppId();
            final ConfigModel configuration = this.configService.loadApplicationConfiguration(context.getBapId(), "track");
            final boolean authenticate = configuration.getMatchedApi().isHeaderAuthentication();
            OnTrackControllerBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnTrackControllerBuyer.class);
    }
}
