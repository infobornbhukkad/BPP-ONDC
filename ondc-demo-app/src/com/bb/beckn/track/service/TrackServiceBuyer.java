...
@Service
public class TrackServiceBuyer
{
    ...
    public ResponseEntity<String> track(final Schema request) throws JsonProcessingException {
        TrackServiceBuyer.log.info("Going to validate json request before sending to seller...");
    ...
    private String sendRequestToSeller(final Schema request) {
        ...
        TrackServiceBuyer.log.info("going to call seller id: {} with action: {}", (Object)context.getBppId(), (Object)action);
        final String url = context.getBppUri();
        final String json = this.jsonUtil.toJson((Object)request);
        TrackServiceBuyer.log.info("final json to be send {}", (Object)json);
        final ConfigModel configModel = this.configService.loadApplicationConfiguration(context.getBapId(), "track");
    ...
    static {
        log = LoggerFactory.getLogger((Class)TrackServiceBuyer.class);
    }
}
