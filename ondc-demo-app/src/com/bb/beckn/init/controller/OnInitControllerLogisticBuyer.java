...
@RestController
public class OnInitControllerLogisticBuyer
{
    ...
    public ResponseEntity<String> onInit(@RequestBody final String body, @RequestHeader final HttpHeaders httpHeaders) throws JsonProcessingException {
    	OnInitControllerLogisticBuyer.log.info("Inside OnInitControllerLogisticBuyer " + body);
    	OnInitControllerLogisticBuyer.log.info("The body in {} adaptor is {}", (Object)"init");
        try {
            final OnSchema model = (OnSchema)this.jsonUtil.toModel(body, (Class)OnSchema.class);
            OnInitControllerLogisticBuyer.log.info("data in buyer controller adaptor [which is will now send back to buyer internal api] is {}", (Object)model.toString());
            ...
            OnInitControllerLogisticBuyer.log.info("does seller {} requires to be authenticated ? {}", (Object)bppId, (Object)authenticate);
            if (authenticate) {
    ...
    static {
        log = LoggerFactory.getLogger((Class)OnInitControllerLogisticBuyer.class);
    }
}
